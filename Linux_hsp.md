...
1. `pwd` (Print Working Directory - 显示当前路径)
   - **作用**：当你迷失方向时，敲一下它，它会告诉你当前所在的确切位置（绝对路径）
   - **示例输出**：`/home/tom/Downloads`
...
如果只敲 ls，它只会平铺显示文件名字。我们通常会搭配参数使用：

```bash
# 看详细信息，也就是常说的 ll（很多系统给 ls -l 做的快捷别名），会显示权限、拥有者、大小、修改时间
ls -l

# 看隐藏文件，在 Linux 里，只要名字以 . 开头的文件（如 .bashrc ）就是隐藏文件，普通的 ls 看不到
ls -a

# 老鸟组合技: 列出所有文件 + 详细信息 + h 代表 Human-readable，把文件大小变成易读的 K、M、G，而不是一堆字节数字
ls -lah
```
...
- **基础用法**：mkdir test_folder
- **🌟 避坑技巧**：如果你想一次性创建很多层文件夹（比如 a/b/c），直接敲会报错“找不到目录”。你必须加上 **-p (parents)** 参数：
  mkdir -p a/b/c （自动把没有的父目录全部建好）。

#### 2. touch (创建空文件 / 摸一下)

- **作用**：创建一个没有任何内容的空文本文件。
- **用法**：touch 1.txt
- *(冷知识：如果 1.txt 已经存在，touch 不会清空它，而是会把它的“最后修改时间”更新到现在，就像用手去“摸”了它一下。)*

#### 3. rm (Remove - 删除文件或目录) ⚠️ 危险命令！

在 Linux 命令行里删除文件，**没有回收站！没有回收站！删了就是永别！**

- **删文件**：rm 1.txt
- **删目录**：必须加上 -r (recursive，递归)，否则删不掉。 rm -r test_folder
- **强删不提示**：-f (force，强制)。
- **☠️ 禁忌魔法**：**rm -rf /\***（强制递归删除根目录下的所有文件。敲完这行命令，你的系统会在 5 秒内灰飞烟灭。**千万别试！**）
...
- 
- **复制文件**：cp 源文件 目标路径 （例：cp 1.txt /tmp/）
- **复制目录**：和 rm 一样，涉及到目录必须加 **-r**。 （例：cp -r folder1 /tmp/）

#### 2. mv (Move - 移动 / 重命名)

这是一个非常奇妙的命令，它既管“剪切”，又管“重命名”。

- 
- **移动位置**：mv 1.txt /tmp/ （把文件剪切到 /tmp 目录下）
- **重命名**：如果目标路径还在当前文件夹，只是名字变了，那它就是重命名。
...
如果是照片、视频，你需要图形界面。但在服务器上，大部分文件都是纯文本（如配置文件、日志），下面这些命令能让你快速阅读它们：

#### 1. cat (全量输出)

- 
- **作用**：把整个文件的内容“吧唧”一下全拍在屏幕上。
- **适用场景**：只适合看**短文件**。如果文件有一万行，屏幕会瞬间滚到底。
- **用法**：cat /etc/passwd
...
- 
- **作用**：用来优雅地看**长文件**。
- **特点**：和上一节讲的 man 命令界面一模一样！按空格翻页，按 / 搜索，按 q 退出。
- **用法**：less /var/log/syslog

#### 3. head 和 tail (掐头去尾)

- 
- **head -n 10 文件名**：只看文件的前 10 行。
- **tail -n 10 文件名**：只看文件的最后 10 行。
- **🌟 运维神技：tail -f 日志文件**
  （-f = follow）。它不仅会显示最后十行，而且**不会退出**，系统一旦往这个文件里写了新日志，你的屏幕就会**实时滚动刷新**！排查程序报错时，这是用得最多的命令，按 Ctrl+C 退出。